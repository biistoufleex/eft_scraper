...
(async () => {
    ...
    const bidouille = await page.evaluate( () => document.querySelector('.no-wrap > .price-main').textContent.length);

    await searchInput.type(searchValue);
    await page.waitForFunction(`document.querySelector('.no-wrap > .price-main').textContent.length !== ${bidouille}`);
